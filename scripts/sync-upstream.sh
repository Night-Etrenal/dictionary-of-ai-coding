...
UPSTREAM_URL="https://github.com/mattpocock/dictionary-of-ai-coding.git"
...
high_risk="$(printf '%s\n' "$changed" | grep -E '^(\.github/workflows/|\.github/actions/|\.codex/|\.agents/|AGENTS\.md$|package(-lock)?\.json$|pnpm-lock\.yaml$|yarn\.lock$|scripts/)' || true)"
...
No upstream code was executed, committed or pushed.

Required next steps:
  1. Review: git diff --cached --stat && git diff --cached
  2. Run the original repository generator and validation commands.
  3. Run the portfolio security workflow or scanner.
  4. Commit and push this branch only after review, then open a draft PR.
